        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {newsStatus === 'succeeded' && articles && articles.length > 0 ? (
            articles.map((article, index) => (
              <div key={`${article.url}-${index}`} className="bg-white rounded-md">
                <Link to={`/articles/${encodeURIComponent(article.title)}`}>
                  <div className="p-8 shadow-[0_3px_10px_rgb(0,0,0,0.2)]">
                    ...
                    {article.urlToImage === null ? (
                      ...
                      <img src={article.urlToImage} alt={article.title} className="w-full h-60 my-4" />
                    )}
            ...
            <div className="flex justify-center items-center min-h-[50vh] col-span-full font-semibold">
